...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        if (runner.t_env - last_stored_data_T) / 10000 >= 1.0:
            import pickle
            total_name_list=["t_env_list","buffer_list","mac_list","learner_list","episode_batch_list"]
            total_list=[t_env_list,buffer_list,mac_list,learner_list,episode_batch_list]
            for i in range(len(total_name_list)):
                os.makedirs("../results/offline_data/{}".format(logger.directory_name), exist_ok=True)
                output_hal = open("../results/offline_data/{}/{}_{}_{}.pkl".format(logger.directory_name,t_env_list, last_stored_data_T, runner.t_env), 'wb')
                str = pickle.dumps(total_list[i])
