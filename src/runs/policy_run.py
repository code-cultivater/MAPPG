...
def run(_run, _config, _log, pymongo_client=None):
    ...
    if(args.env=="sc2"):
        ...
        raise Exception("error in run.py 193 line")
        ...
        if buffer.can_sample(args.batch_size):
            if(args.critic_update_ind):
                ...
                for _ in range(args.training_iters):
                    ...
                    # if(learner.adv_max_rho>0.1):
                    #     learner.num_actor_updates += 1
                    #     learner.num_actor_updates = min(5, learner.num_actor_updates)
                    # else:
                    #     learner.num_actor_updates -= 1
                    #     learner.num_actor_updates = max(1, learner.num_critic_updates)


                    if (4*learner.critic_loss_rho >  learner.adv_max_rho):
                        learner.num_critic_updates += 1
                        learner.num_critic_updates = min(5, learner.num_critic_updates)
                    else:
                        learner.num_critic_updates -= 1
                        learner.num_critic_updates = max(1, learner.num_critic_updates)

                    if (learner.args.test_return_record > 19):
                        learner.num_critic_updates = 1
                        learner.num_actor_updates = 1
                    learner.logger.log_stat("num_critic_updates", learner.num_critic_updates, runner.t_env)
                    learner.logger.log_stat("num_actor_updates", learner.num_actor_updates, runner.t_env)

                    for _ in range(learner.num_actor_updates):
                        learner.train_actor(episode_sample,runner.t_env,episode)
                    learner.train_critic(episode_sample,runner.t_env,episode)
                    #print("num_critic_updates",learner.num_critic_updates)
                    for _ in range(learner.num_critic_updates-1):
