#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2021/3/15 10:00
# @Author  : Wubing Chen
# @Site    : adapted frome LICA and meta_weight_nets
# @File    : lica.py
# @Software: PyCharm
import torch
...
class LICACritic(nn.Module):
    def __init__(self, args):
        ...
        self.embed_dim = args.mixing_embed_dim * self.n_agents * self.n_actions#mixing_embed_dim*n_agents*n_actions
    ...
    def forward(self, act, states):
        ...
        action_probs = act.reshape(-1, 1, self.n_agents * self.n_actions)#[bs*ts,1,n_agents*n_actions]

        w1 = self.hyper_w_1(states)
        b1 = self.hyper_b_1(states)
        w1 = w1.view(-1, self.n_agents * self.n_actions, self.hid_dim)#mixing_embed_dim*n_agents*n_actions->(1,n_agents*n_actions,mixing_embed_dim)
        b1 = b1.view(-1, 1, self.hid_dim)

        h = torch.relu(torch.bmm(action_probs, w1) + b1)#(128,1,6)*(128,6,64)+（128，1，64）
