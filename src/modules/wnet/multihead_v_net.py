...
class MultiheadVNet(nn.Module):
    def __init__(self, args):
        super(MultiheadVNet, self).__init__()

        self.args = args
        self.n_head_agent=self.args.n_head_agent
        ...
        if getattr(args, "hypernet_layers", 1) == 1:
            ...
            self.hyper_w_final = nn.Sequential(nn.Linear(self.state_dim, self.hid_dim),
                                           nn.ReLU(),
                                           nn.Linear(self.hid_dim, self.hid_dim*self.n_head_agent))
        elif getattr(args, "hypernet_layers", 1) > 2:
        ...
        self.hyper_b_2 = nn.Sequential(nn.Linear(self.state_dim, self.hid_dim),
                               nn.ReLU(),
                               nn.Linear(self.hid_dim, self.n_head_agent))
    ...
    def forward(self, act, states):#act torch.Size([32, 47, 5, 12])
        ...
        w_final = w_final.view(-1, self.hid_dim, self.n_head_agent)

        h2 = torch.bmm(h, w_final)

        b2 = self.hyper_b_2(states).view(-1, 1, self.n_head_agent)

        q = h2 + b2

        q=self.bn(q)
        q = torch.softmax(q,dim=-1)  # [32*47,self.n_head_agent]
        #q_sum=torch.sum(q,dim=-1,keepdim=True)# [32*47,1]
        #q=q/q_sum
        q = q.view(bs, -1, self.n_head_agent)
